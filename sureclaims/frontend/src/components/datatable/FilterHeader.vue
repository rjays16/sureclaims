<template>
  <div class="btn-group">
    ...
    <a href="javascript:;" data-toggle="dropdown">
      <i class="la la-filter" :class="{ 'text-muted': !keyword }"></i>
    </a>
    <ul class="dropdown-menu" style="padding: 3px">
      <div class="input-group input-group-sm">
        ...
        <span class="input-group-btn">
            <button class="btn btn-default la la-search" @click="search"></button>
          </span>
      </div>
    </ul>
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      search() {
        const { query } = this;
        // `$props.query` would be initialized to `{ limit: 10, offset: 0, sort: '', order: '' }`
        // by default
